...
public class GClickThrough extends Extension {
    ...
    protected void initExtension() {
        ...
        hashSupport.intercept(HMessage.Direction.TOCLIENT, "YouArePlayingGame", new MessageListener() {
        ...
        chatConsole.onInput(new ChatInputListener() {
            public void inputEntered(String s) {
                if (s.equals(":start")) {
                    isStart = true;
                    hashSupport.sendToClient("YouArePlayingGame", true);
                }
                else if (s.equals(":stop")) {
                    isStart = false;
                    hashSupport.sendToClient("YouArePlayingGame", false);
                }
            }
        });
